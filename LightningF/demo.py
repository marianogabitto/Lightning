...
# from Models.pdp_simple import PDpTPoissGausDiagCovSimpleDir as Model
...
for i_ in np.arange(new.shape[0]):
    ...
    modelpy[i_].fit_moves(iterations=100, pl=0, prt=1, which_moves=[True, True, True, True])
    modelpy[i_].print_number_moves()
    modelc[i_].fit_moves(iterations=100, pl=0, prt=1, which_moves=[True, True, True, True])
    modelc[i_].print_number_moves()
    modelpy[i_].pl_bl()
    modelc[i_].pl_bl()
    # model[i_].pl_bl(circles=True)
# ###################################################################################################
print(1)
